...
export default function SettingsDrawer({ open, onClose, prices, onUpdatePrices, readOnly }: SettingsDrawerProps) {
  const [regularPrice, setRegularPrice] = useState('');
  const [premiumPrice, setPremiumPrice] = useState('');

  useEffect(() => {
    if (open) {
      setRegularPrice((prices['regular'] || 0).toString());
      setPremiumPrice((prices['premium'] || 0).toString());
    }
  }, [open, prices]);
          ...
          >
            ...
            <div className="p-5 space-y-5">
              <p className="text-slate-400 text-sm">
                {readOnly
                  ? 'Precios actuales configurados por el encargado.'
